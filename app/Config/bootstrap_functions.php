...
/**
 * Generic function for getting model's field general information.
 * 
 * @param  mixed $model     Instance of your Model class or a model string from where to pull the info.
 * @param  string $field    Column/Field name.
 * @param  string $dataType Type of the data to fetch.
 * @return string           Result.
 *
 * @deprecated in favor of FieldDataEntity
 */
function getFieldData($model, $field, $dataType, $options = array()) {
	$_m = _getModelInstance($model);
	
	$fieldLabel = call_user_func_array(array($_m, 'fieldData'), array($field, $dataType, $options));
	return $fieldLabel;
}
...
function getLdapConnectorEmailFetchTypes() {
	return array(
		LDAP_CONNECTOR_EMAIL_FETCH_EMAIL_ATTRIBUTE => __('Email Attribute'),
		LDAP_CONNECTOR_EMAIL_FETCH_ACCOUNT_DOMAIN => __('Account plus Domain')
	);
}
...
function getPoliciesDocumentPermissions() {
	return array(
		SECURITY_POLICY_PUBLIC => __('Public'),
		SECURITY_POLICY_PRIVATE => __('Private'),
		SECURITY_POLICY_LOGGED => __('Authorized Uses Only'),
...
function getCustomFieldTypes($type = null) {
	$types = array(
		CUSTOM_FIELD_TYPE_SHORT_TEXT => __('Short Text'),
		CUSTOM_FIELD_TYPE_TEXTAREA => __('Paragraph'),
		CUSTOM_FIELD_TYPE_DATE => __('Date'),
		CUSTOM_FIELD_TYPE_DROPDOWN => __('Dropdown'),
	);
...
function getCustomFieldArg($field) {
	return 'customField__' . $field['slug'];
}
