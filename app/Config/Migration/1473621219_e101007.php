 ...
 */
	...
	private function manageDataUp() {
		...
		// $ret &= $this->updateRiskScores();

		return $ret;
	}

	/**
	 * Use generic saving callbacks to recalculate risk scores after it's logic was updated for this release.
	 */
	private function updateRiskScores() {
		$ret = true;

		// risks
		$Risk = $this->initModel('Risk');
		$risks = $Risk->find('list', array(
			'fields' => array('id', 'id'),
			'recusive' => -1
		));
		...
		foreach ($risks as $riskId) {
			...
			$ret &= $Risk->beforeItemSave($Risk->id);
			$ret &= $Risk->afterItemSave($Risk->id);
		}

		// third party risks
		$ThirdPartyRisk = $this->initModel('ThirdPartyRisk');
		$risks = $ThirdPartyRisk->find('list', array(
			'fields' => array('id', 'id'),
			'recusive' => -1
		));

		foreach ($risks as $riskId) {
			$ThirdPartyRisk->id = $riskid;

			$ret &= $ThirdPartyRisk->beforeItemSave($ThirdPartyRisk->id);
			$ret &= $ThirdPartyRisk->afterItemSave($ThirdPartyRisk->id);
		}

		// business continuity
		$BusinessContinuity = $this->initModel('BusinessContinuity');
		$risks = $BusinessContinuity->find('list', array(
			'fields' => array('id', 'id'),
			'recusive' => -1
		));

		foreach ($risks as $riskId) {
			$BusinessContinuity->id = $riskid;

			$ret &= $BusinessContinuity->beforeItemSave($BusinessContinuity->id);
			$ret &= $BusinessContinuity->afterItemSave($BusinessContinuity->id);
		}
