			...
			<div class="widget-content">
				<div class="tabbable box-tabs box-tabs-styled <?= (!empty($filter['settings']['scrollable_tabs'])) ? 'box-tabs-2-rows' : '' ?>">
					...
					<div id="advanced-filter-tabs">
						<?php 
						$formUrl = (!empty($filter['settings']['url'])) ? $filter['settings']['url'] : array('action' => 'index');
						if (!empty($this->request->data[$filter['model']]['id'])) {
							unset($this->request->data[$filter['model']]['id']);
						}
						echo $this->Form->create($filter['model'], array(
							...
							if (!empty($activeFilter)) {
								echo $this->Form->input('advanced_filter_id', array(
									'type' => 'hidden',
									'value' => $activeFilter['AdvancedFilter']['id']
								));
							}
							...
							<?php foreach ($filter['fields'] as $key => $fieldSet) : ?>
								...
								<div class="tab-pane fade in <?php echo ($counter == 0 && !isset($successMessage) && !isset($errorMessage)) ? 'active' : ''; ?>" id="advanced-filter-tab-<?php echo $counter ?>">
									...
									foreach ($fieldSet as $field => $fieldData) {
										$elem = (!empty($filter['settings']['use_new_filters']) && isset($fieldData['filter']['method']) && $fieldData['filter']['method'] == 'findComplexType') ? 'input_complex' : 'input';
							...
							<?php endforeach; ?>
							<?php
							echo $this->Form->input('_limit', array(
								'type' => 'hidden',
								'default' => ADVANCED_FILTER_DEFAULT_PAGE_LIMIT,
								'id' => 'advanced-filter-limit-input'
							));
							?>
