					...
					<table class="table-pdf table-pdf-system-records" style="">
						...
						<tbody>
							<?php foreach ($items as $item) : ?>
								<tr>
									<td><?php echo $item['Risk']['title']; ?></td>
									<td><?= $this->UserField->showUserFieldRecords($item['Stakeholder']); ?></td>
									<td><?php echo $item['RiskMitigationStrategy']['name']; ?></td>
									<td><?php echo $item['Risk']['risk_score']; ?></td>
									<td><?php echo $item['Risk']['residual_risk']; ?></td>
									<td><?php echo $item['Risk']['review']; ?></td>
									<td>
										<?php
										echo $this->Risks->getStatuses($item, 'Risk', true);
										?>
									</td>
								</tr>
							<?php endforeach; ?>
