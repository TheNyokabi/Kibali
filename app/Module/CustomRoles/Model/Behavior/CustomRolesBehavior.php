...
class CustomRolesBehavior extends ModelBehavior {
	...
	public function getCustomRolesLabels(Model $Model) {
		$list = [];
		...
		foreach ($customRoles as $field) {
			...
			if ($Model instanceof InheritanceInterface) {
				...
				if ($parent !== null && $Model->{$parent}->hasMethod('getCustomRolesLabels')) {
					$list = am($list, $Model->{$parent}->getCustomRolesLabels());
				}
			}
		}
