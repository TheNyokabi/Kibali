...
class UserFieldsBehavior extends ModelBehavior
{
	...
	protected $userIdPrefix = 'User-';
	protected $groupIdPrefix = 'Group-';
	 ...
	 * @return array.        Names of associations
	 */
	protected function getAllAssociations(Model $model)
	{
		$associations = [];
		foreach ($this->settings[$model->alias]['fields'] as $field) {
	 ...
	 */
	protected function getUserIdPrefix(Model $model)
	{
	 ...
	 */
	protected function getGroupIdPrefix(Model $model)
	{
	...
	public function beforeSave(Model $model, $options = array())
	{
		...
		foreach ($habtmAssocs as $habtmAssoc) {
			...
			foreach ($model->data[$habtmAssoc][$habtmAssoc] as $key => $val) {
				...
				if (is_string($val) && strpos($val, $this->groupIdPrefix) === 0) {
			...
			foreach ($associations as $assoc) {
				...
				foreach ($model->data[$assoc][$assoc] as $key => $val) {
					if (is_string($val) && (strpos($val, $this->userIdPrefix) === 0 || strpos($val, $this->groupIdPrefix) === 0)) {
						$newRecord = [];
		...
		foreach ($data as $r_key => $record) {
			if (isset($record[$userFieldName])) {
				foreach ($record[$userFieldName] as $u_key => $user) {
					...
					$user['id'] = $this->getUserIdPrefix($model) . $user['id'];
		...
		foreach ($data as $r_key => $record) {
			if (isset($record[$groupFieldName])) {
				foreach ($record[$groupFieldName] as $group) {
					$group['id'] = $this->getGroupIdPrefix($model) . $group['id'];
	...
	public function addUserFieldToDb(Model $model, $recordId, $field, $data)
	{
		...
		foreach ($data as $d) {
			if (strpos($d, $this->getUserIdPrefix($model)) === 0) {
	...
	public function getUsersGroupsOptions(Model $model)
	{
		...
		foreach ($usersData as $id => $name) {
			$usersOptions[$this->userIdPrefix . $id] = $name;
		}
		//
		...
		foreach ($groupsData as $id => $name) {
			$groupsOptions[$this->groupIdPrefix . $id] = $name;
		}
		//
		...
		foreach ($tempData as $tData) {
			...
			foreach ($tData[$userFieldAssoc] as $tempUser) {
				if (array_key_exists('id', $tempUser)) {
					if (strpos($tempUser['id'], $this->userIdPrefix) === 0) {
						$userId = (int)explode('-', $tempUser['id'])[1];
						$tempUser['id'] = $userId;
						$tempUsers[$id][$userId] = $tempUser;
					} elseif (strpos($tempUser['id'], $this->groupIdPrefix) === 0 && array_key_exists('User', $tempUser)) {
