...
class UserFields
{
	...
	public function getFieldDataEntityData($model, $field, $options = [])
	{
		...
		$editableData = ['label', 'editable', 'description', 'empty', 'group'];
	...
	public function getAdvancedFilterFieldData($modelAlias, $field, $options = [])
	{
		...
		$editableData = ['name', 'show_default'];
		return $this->replaceEditableData($data, $options, $editableData);
	}
	 ...
	 */
	public function getImportArgsFieldData($field, $options, $optional = false)
	{
		$headerTooltipPrefix = "";
		if (!array_key_exists('headerTooltip', $options) && $optional == true) {
			$headerTooltipPrefix = __('Optional') . '. ';
		}
		$headerTooltipPrefix .= __('Accepts multiple IDs separated by "|". For User IDs use prefix "User-" and for Group IDs use "Group-". For example "User-1|Group-10|Group-11"') . '. ';
		$data = [
			'name' => __('Owner'),
			'model' => $field,
			'headerTooltip' => $headerTooltipPrefix . __('You can get the ID of an user account from System / Settings / User Management or ID of a group from System / Settings / Groups.')
		];
	 ...
	 */
	private function replaceEditableData($data, $options, $editableData)
	{
		foreach ($options as $key => $val) {
			if (in_array($key, $editableData)) {
				$data[$key] = $val;
			} elseif (array_key_exists($key, $editableData) && is_array($editableData[$key])) {
				$data[$key] = $this->replaceEditableData($data[$key], $options[$key], $editableData[$key]);
			}
		}
