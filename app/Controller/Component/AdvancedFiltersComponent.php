...
class AdvancedFiltersComponent extends Component {
	...
	public $components = array('Search.Prg', 'Paginator', 'AdvancedFiltersExtented', 'Crud.Crud');
	...
	public function initialize(Controller $controller) {
		...
		$this->Prg->initialize($controller);

		$this->AdvancedFiltersExtented->initialize($controller);
	}

	private function setSettings($model = null) {
		if ($model === null) {
			$model = $this->model;
		}
	...
	protected function _setRender($type) {
		...
		if ($this->_Collection->enabled('Crud')) {
			$this->Crud->trigger('beforeRender', array('success' => true));
		}
	...
	/*public function trash($model, $view = 'html') {
		$this->model = $model;
		...
		if (empty($this->controller->request->query['advanced_filter'])) {
			$this->Prg->commonProcess($this->model, array(
				'filterEmpty' => true,
				'excludedParams' => $this->cleanDefaultsBeforeProcess()
			));
		...
		$fields = $this->getFields(($selectionSize < 1) ? true : false);

		$options = $this->getFindOptions($conditions, $fields);
		// trash
		$this->Paginator->settings['conditions'][$this->model . '.deleted'] = true;

		$data = $this->_getData($view, $options);
		...
		if (!empty($this->activeFilter)) {
			$this->controller->request->data['Edit'] = $this->activeFilter;
		}
		...
		$this->controller->render(DS . 'Elements' . DS . ADVANCED_FILTERS_ELEMENT_PATH . 'index');
	...
	private function getFields($setDefault = false) {
		...
		if ($this->AdvancedFiltersExtented->hasCustomFields()) {
		...
		foreach ($this->settings['advancedFilter'] as $fieldSet) {
			foreach ($fieldSet as $field => $fieldData) {
				...
				if (!empty($fieldData['data']['method']) || !empty($fieldData['data']['callable'])) {
					...
					$this->controller->set($field . '_data', $data);
				}
